...
bool __fastcall XRSACrypt::GetPublicReady(void) {
	// Vérifier non seulement si le blob existe, mais aussi s'il s'agit d'une clé publique valide
	return (publicblob.size() > 0 && IsPublicBlob(publicblob) && IsValid(publicblob));
}

bool __fastcall XRSACrypt::GetPrivateReady(void) {
	// Vérifier non seulement si le blob existe, mais aussi s'il s'agit d'une clé privée valide
	return (privateblob.size() > 0 && IsPrivateBlob(privateblob) && IsValid(privateblob));
}
...
bool __fastcall XRSACrypt::IsValid(const std::vector<BYTE> &buffer) {
	...
	// Vérifier la structure BCRYPT_RSAKEY_BLOB
	...
	// Déterminer le type de blob en fonction de la valeur Magic
	...
	} else if (keyBlob->Magic == BCRYPT_RSAPRIVATE_MAGIC) {
	...
	// Tenter d'importer la clé avec le type de blob correct
	status = BCryptImportKeyPair(hAlg, NULL, blobType, &hKey, const_cast<BYTE*>(buffer.data()), buffer.size(), 0);
...
//---------------------------------------------------------------------------
// Extract Blob Private/Public from hkey
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
bool __fastcall XRSACrypt::NewKeyPair(void) {
	NTSTATUS status  = -1;
	ULONG    keysize =  0;
...
bool __fastcall XRSACrypt::LoadBlob(const UnicodeString& filename) {
	std::vector<BYTE> blob;

	// Charger le blob depuis le fichier
	if (!FileToBuffer(filename, blob) || blob.empty()) return false;

	// Nettoyer les clés existantes avant de charger une nouvelle clé
	ClearContext();

	if (IsPrivateBlob(blob)) {
		if (SetPrivateBlob(blob)) return ExtractPublicBlob();
	}
	else if (IsPublicBlob(blob)) {
		if (SetPublicBlob(blob)) return true;
	}

	return false;
}

bool __fastcall XRSACrypt::SaveBlobPrivate(const std::string &filename) {
...
bool __fastcall XRSACrypt::SaveBlobPublic(const std::string &filename) {
...
bool __fastcall XRSACrypt::SetPublicBlob(const std::vector<BYTE>& blob) {
	...
	// Nettoyer complètement toutes les clés existantes
	ClearContext();

	// Stocker le blob public
	publicblob = blob;
	...
	// Vérification finale que la clé privée est bien vide
	if (privateblob.size() > 0) {
		privateblob.clear();
	}
...
UnicodeString __fastcall XRSACrypt::EncryptString(const UnicodeString& str) {
	...
	try {
		...
		if ( EncryptBuffer(buff, encryptedBuffer) ) return BufferToHex(encryptedBuffer);
...
UnicodeString __fastcall XRSACrypt::DecryptString(const UnicodeString& hexstr) {
	// Vérification stricte que nous avons une clé privée valide
	if (!PrivateReady) { return L"Erreur: Clé privée non initialisée"; }
	
	// Double vérification que nous avons bien une clé privée et non une clé publique
	if (!IsPrivateBlob(privateblob)) { return L"Erreur: Clé privée invalide ou non disponible"; }
	...
	try {
		...
		if ( DecryptBuffer(buff, decryptedBuffer) ) return BufferToUnicode(decryptedBuffer);
...
bool __fastcall XRSACrypt::DecryptFile(const UnicodeString& infile, const UnicodeString& outfile) {
	...
	// Double vérification que nous avons bien une clé privée et non une clé publique
	if (!IsPrivateBlob(privateblob)) { return false; } // Erreur: Clé privée invalide ou non disponible
...
bool __fastcall XRSACrypt::DecryptBuffer(const std::vector<BYTE>& encrypteddata, std::vector<BYTE>& uncrypt) {
	...
	// Double vérification que nous avons bien une clé privée et non une clé publique
	if (!IsPrivateBlob(privateblob)) return false; // Erreur: Le blob n'est pas une clé privée
